...
import audio_config as audio_cfg
import audio_utils
...
def record_and_transcribe(
    ...
    Args:
      silence_threshold:
        Lower = more sensitive. Default audio_cfg.SILENCE_THRESHOLD.
    ...
    silence_threshold = (
        ...
        else int(audio_cfg.SILENCE_THRESHOLD)
    )

    silence_duration = (
        float(silence_duration)
        if silence_duration is not None
        else float(audio_cfg.SILENCE_DURATION)
    )

    max_record_seconds = (
        int(max_record_seconds)
        if max_record_seconds is not None
        else int(audio_cfg.MAX_RECORD_SECONDS)
    )

    recognizer = KaldiRecognizer(model, int(audio_cfg.VOSK_SAMPLE_RATE))
    ...
    try:
        with sd.RawInputStream(
            ...
            for _ in range(max_chunks):
                ...
                if int(mic_rate) != int(audio_cfg.VOSK_SAMPLE_RATE):
                    resampled = resample_audio(
                        audio_chunk,
                        int(mic_rate),
                        int(audio_cfg.VOSK_SAMPLE_RATE),
                    )
                    utterance_done = recognizer.AcceptWaveform(resampled.tobytes())
                else:
                    utterance_done = recognizer.AcceptWaveform(data)

                # Vosk's built-in endpointing: AcceptWaveform returns True
                # when Vosk decides the utterance is finished — smarter than
                # pure silence timing, so we can stop listening earlier.
                if (
                    utterance_done
                    and getattr(audio_cfg, "USE_VOSK_ENDPOINTING", True)
                    and frames_collected > 10
                ):
                    break
